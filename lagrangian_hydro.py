...
class LagrangianHydro:
    ...
    def solveVelocityPredictor(self, dt):
        ...
        if self.constrain_u:
            u_p[0], u_p[-1] = self.fields.u_BC
        # Pressure BC, use Eqs. 37 and 38
        else:
            P_L, P_R = self.fields.P_BC
            E_L, E_R = self.computeE_BCs(E_old)
            # Left cell
            coeff = -A_old[0] * dt / m_half[0]
            u_p[0] = u_old[0] + coeff * (P_old[0] - P_L + (E_old[0] - E_L) / 3)
            # Right cell
            coeff = -A_old[-1] * dt / m_half[-1]
            u_p[-1] = u_old[-1] + coeff * (P_old[-1] - P_R + (E_old[-1] - E_R) / 3)

        # Sweep to the right for each interior median mesh cell
        for i in range(1, self.N - 1):
            coeff = -A_old[i] * dt / m_half[i]
            u_p[i] = u_old[i] + coeff * (P_old[i + 1] - P_old[i])
            u_p[i] += coeff * (E_old[i + 1] - E_old[i]) / 3
    ...
    def computeE_BCs(self, E):
        self.mat.recomputeKappa_t(self.fields.T_old)
        ...
        # Source BC (Use Eqs. 39 and 40)
            ...
            E_L = 3 * rho_old[0] * dr_L * kappa_t[0] * E_BC_L + 4 * E[0]
            E_L /= 3 * rho_old[0] * dr_L * kappa_t[0] + 4
            E_R = 3 * rho_old[-1] * dr_R * kappa_t[-1] * E_BC_R + 4 * E[-1]
            E_R /= 3 * rho_old[-1] * dr_R * kappa_t[-1] + 4
